...
private:
    struct SolverParams {
        ...
        // Qband anti-excitation term: penalize predicted lateral acceleration energy around the slosh dominant band.
        bool enable_qband_penalty = true;
        bool enable_qband_soft_constraint = true;
        bool enable_qband_hard_constraint = false;
        bool enable_preview_qband_weight = true;
        double qband_center_frequency = 1.50;      // Hz, model tank dominant coupled slosh frequency
        double qband_bandwidth_frequency = 0.55;   // Hz, approximate -3 dB bandwidth
        double qband_weight_base = 8.0;
        double qband_preview_scale_min = 0.30;
        double qband_preview_scale_max = 8.0;
        double qband_preview_ay_threshold = 0.55;  // m/s^2
        double qband_preview_ay_range = 0.90;      // m/s^2
        double qband_energy_limit = 0.20;          // (m/s^2)^2, average band energy
        double qband_energy_soft_weight = 80.0;
        double qband_softplus_eps = 1e-6;
        double qband_hard_constraint_slack = 0.05;
    } p_;
...
private:
    ...
    double computePreviewQbandWeight(double vx_now, const casadi::DM& waypoints) const;
    void computeQbandCoefficients();
...
private:
    ...
    casadi::MX qband_weight_param_;

    double qband_b0_ = 0.0;
    double qband_b1_ = 0.0;
    double qband_b2_ = 0.0;
    double qband_a1_ = 0.0;
    double qband_a2_ = 0.0;
