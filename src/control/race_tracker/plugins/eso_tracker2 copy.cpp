...
bool ESOTracker2::initialize(ros::NodeHandle& nh) {
    ...
    nh_nmpc.param("Q_delta", nmpc_params_.Q(5,5), 1.0);      // 未使用该权重项
    nh_nmpc.param("Q_r_t", nmpc_params_.Q(6,6), 3000.0);      // 挂车状态权重
    nh_nmpc.param("Q_gamma", nmpc_params_.Q(7,7), 2000.0);      // 挂车状态权重
    nh_nmpc.param("Q_dgamma", nmpc_params_.dgamma, 5000.0);      // 挂车状态权重
