...
bool NMPCController::initialize(ros::NodeHandle& nh) {
    ...
    nh.param("nx", nx_, 6);
    nh.param("nu", nu_, 3);
    nh.param("prediction_step", N_, 20);
    nh.param("sparse_control_step", Nc_, 4);
    nh.param("front_steer_time_constant", T_d1_, 0.2);
    nh.param("rear_steer_time_constant", T_d2_, 0.2);
    nh.param("sampling_time", dt_, 0.1);
    nh.param("wheelbase", L_, 2.8);
    nh.param("gravity", g_, 9.806);
    nh.param("max_acceleration", a_max_, 3.0);
    nh.param("num_waypoints", n_waypoints_, 30);

    // 加载控制量边界
    nh.param("min_acceleration", ax_min_, -1.0);
    nh.param("max_acceleration", ax_max_, 1.0);
    nh.param("min_front_steer", delta1_min_, -0.4);
    nh.param("max_front_steer", delta1_max_, 0.4);
    nh.param("min_rear_steer", delta2_min_, -0.001);
    nh.param("max_rear_steer", delta2_max_, 0.001);
    nh.param("max_speed", max_speed_, 20.0);

    // 加载代价函数权重
    nh.param("weight_position", w_pos_, 5.0);
    nh.param("weight_heading", w_theta_, 3.0);
    nh.param("weight_velocity", w_v_, 5.0);
    nh.param("weight_acceleration", w_ax_, 5.0);
    nh.param("weight_front_steer", w_delta1_, 500.0);
    nh.param("weight_front_steer_control", w_delta_cmd1_, 1.0);
    nh.param("weight_rear_steer", w_delta2_, 0.2);
    nh.param("weight_terminal_position", w_term_pos_, 10.0);
    nh.param("weight_terminal_heading", w_term_theta_, 1.0);
    nh.param("weight_terminal_velocity", w_term_v_, 5.0);
    nh.param("weight_rear_steer_control", w_delta_cmd2_, 1.0);

    // 打印加载的参数
    logParamLoad("nx", nx_, 6);
    logParamLoad("nu", nu_, 3);
    logParamLoad("prediction_step", N_, 20);
    logParamLoad("sparse_control_step", Nc_, 4);
    logParamLoad("front_steer_time_constant", T_d1_, 0.3);
    logParamLoad("rear_steer_time_constant", T_d2_, 0.3);
    logParamLoad("sampling_time", dt_, 0.1);
    logParamLoad("wheelbase", L_, 2.8);
    logParamLoad("max_acceleration", a_max_, 6.0);
...
void NMPCController::computeControl(
    ...
    if (!solveNMPC(current_state, waypoints_dm, control_output)) {
        ROS_WARN("[%s] NMPC求解失败，使用默认控制量", getName().c_str());
        return;
    }
...
bool NMPCController::solveNMPC(const std::vector<double>& current_state, const casadi::DM& waypoints,
    ...
    try {
        ...
        control_output[0] = static_cast<double>(u0(0));  // 加速度
