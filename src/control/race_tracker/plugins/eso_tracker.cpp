...
bool ESOTracker::initialize(ros::NodeHandle& nh) {
    // 1. 打印原命名空间（确认基础路径）
    ROS_INFO("[ESOTracker] 父NodeHandle命名空间: %s", nh.getNamespace().c_str());
    // 2. 创建插件专属的子NodeHandle（匹配Launch中的ns="eso_tracker"）
    ros::NodeHandle nh_nmpc(nh, "eso_tracker");  // 子命名空间：父ns + "/eso_tracker"
    ROS_INFO("[ESOTracker] 插件专属NodeHandle命名空间: %s", nh_nmpc.getNamespace().c_str());
...
std::vector<double> ESOTracker::vehicleStatusToStateVector(const race_msgs::VehicleStatus& status) {
