...
bool NMPCAntiExitationController::initialize(ros::NodeHandle& nh) {
    ...
    ROS_INFO("[%s] hybrid-fast Qband NMPC initialized: N=%d, control_dim=%d, sparse=%s, low_speed_pp=%s",
...
bool NMPCAntiExitationController::loadParams(ros::NodeHandle& nh) {
    ...
    readParam(nh, "enable_qband_penalty", p_.enable_qband_penalty);
    readParam(nh, "enable_qband_soft_constraint", p_.enable_qband_soft_constraint);
    readParam(nh, "enable_qband_hard_constraint", p_.enable_qband_hard_constraint);
    readParam(nh, "enable_preview_qband_weight", p_.enable_preview_qband_weight);
    readParam(nh, "qband_center_frequency", p_.qband_center_frequency);
    readParam(nh, "qband_bandwidth_frequency", p_.qband_bandwidth_frequency);
    readParam(nh, "qband_weight_base", p_.qband_weight_base);
    readParam(nh, "qband_preview_scale_min", p_.qband_preview_scale_min);
    readParam(nh, "qband_preview_scale_max", p_.qband_preview_scale_max);
    readParam(nh, "qband_preview_ay_threshold", p_.qband_preview_ay_threshold);
    readParam(nh, "qband_preview_ay_range", p_.qband_preview_ay_range);
    readParam(nh, "qband_energy_limit", p_.qband_energy_limit);
    readParam(nh, "qband_energy_soft_weight", p_.qband_energy_soft_weight);
    readParam(nh, "qband_softplus_eps", p_.qband_softplus_eps);
    readParam(nh, "qband_hard_constraint_slack", p_.qband_hard_constraint_slack);
    return true;
}
...
void NMPCAntiExitationController::validateParams() {
    ...
    const double nyquist = 0.5 / p_.dt;
    p_.qband_center_frequency = clamp(p_.qband_center_frequency, 0.05, 0.85 * nyquist);
    p_.qband_bandwidth_frequency = clamp(p_.qband_bandwidth_frequency, 0.05, std::max(0.06, 0.9 * p_.qband_center_frequency));
    p_.qband_weight_base = std::max(0.0, p_.qband_weight_base);
    p_.qband_preview_scale_min = std::max(0.0, p_.qband_preview_scale_min);
    p_.qband_preview_scale_max = std::max(p_.qband_preview_scale_min, p_.qband_preview_scale_max);
    p_.qband_preview_ay_threshold = std::max(0.0, p_.qband_preview_ay_threshold);
    p_.qband_preview_ay_range = std::max(1e-3, p_.qband_preview_ay_range);
    p_.qband_energy_limit = std::max(0.0, p_.qband_energy_limit);
    p_.qband_energy_soft_weight = std::max(0.0, p_.qband_energy_soft_weight);
    p_.qband_softplus_eps = std::max(1e-12, p_.qband_softplus_eps);
    p_.qband_hard_constraint_slack = std::max(0.0, p_.qband_hard_constraint_slack);
...
void NMPCAntiExitationController::buildSingleShootingProblem() {
    ...
    qband_weight_param_ = opti_.parameter();
    ...
    casadi::MX ay_prev1 = 0.0;
    casadi::MX ay_prev2 = 0.0;
    casadi::MX band_prev1 = 0.0;
    casadi::MX band_prev2 = 0.0;
    casadi::MX qband_energy_sum = 0.0;
    ...
    for (int k = 0; k < p_.N; ++k) {
        ...
        const casadi::MX ay_k = vx0_ * vx0_ * steer_fun(delta) / p_.L;
        if (p_.enable_qband_penalty || p_.enable_qband_soft_constraint || p_.enable_qband_hard_constraint) {
            const casadi::MX band_k = qband_b0_ * ay_k + qband_b1_ * ay_prev1 + qband_b2_ * ay_prev2
                                      - qband_a1_ * band_prev1 - qband_a2_ * band_prev2;
            qband_energy_sum += band_k * band_k;
            ay_prev2 = ay_prev1;
            ay_prev1 = ay_k;
            band_prev2 = band_prev1;
            band_prev1 = band_k;
        }
    ...
    if (p_.enable_qband_penalty || p_.enable_qband_soft_constraint || p_.enable_qband_hard_constraint) {
        const casadi::MX qband_energy_avg = qband_energy_sum / static_cast<double>(std::max(1, p_.N));
        if (p_.enable_qband_penalty) {
            cost += qband_weight_param_ * qband_energy_avg;
        }
        if (p_.enable_qband_soft_constraint && p_.qband_energy_soft_weight > 0.0) {
            const casadi::MX violation = qband_energy_avg - p_.qband_energy_limit;
            const casadi::MX pos_violation = 0.5 * (violation + casadi::MX::sqrt(violation * violation + p_.qband_softplus_eps));
            cost += p_.qband_energy_soft_weight * pos_violation * pos_violation;
        }
        if (p_.enable_qband_hard_constraint) {
            opti_.subject_to(qband_energy_avg <= p_.qband_energy_limit + p_.qband_hard_constraint_slack);
        }
    }
...
void NMPCAntiExitationController::computeQbandCoefficients() {
    // RBJ biquad band-pass filter, constant 0 dB peak gain.
    // y[n] = b0*x[n] + b1*x[n-1] + b2*x[n-2] - a1*y[n-1] - a2*y[n-2]
    const double fs = 1.0 / std::max(1e-6, p_.dt);
    const double f0 = clamp(p_.qband_center_frequency, 0.05, 0.45 * fs);
    const double bw = std::max(0.05, p_.qband_bandwidth_frequency);
    const double Q = std::max(0.10, f0 / bw);
    ...
    ROS_INFO("[%s] Qband filter: f0=%.3f Hz, bandwidth=%.3f Hz, Q=%.3f, coeff=[%.6f %.6f %.6f %.6f %.6f]",
             getName().c_str(), f0, bw, Q, qband_b0_, qband_b1_, qband_b2_, qband_a1_, qband_a2_);
}

double NMPCAntiExitationController::computePreviewQbandWeight(double vx_now, const casadi::DM& waypoints) const {
    if (!p_.enable_qband_penalty || !p_.enable_preview_qband_weight) {
        return p_.qband_weight_base;
    }

    double max_abs_ay_ref = 0.0;
    double sum_sq_ay_ref = 0.0;
    int count = 0;
    for (int k = 0; k < p_.N; ++k) {
        const double yaw0 = static_cast<double>(waypoints(2, k));
        const double yaw1 = static_cast<double>(waypoints(2, k + 1));
        const double dyaw = wrapAngle(yaw1 - yaw0);
        const double yaw_rate_ref = dyaw / std::max(1e-6, p_.dt);
        const double ay_ref = vx_now * yaw_rate_ref;
        max_abs_ay_ref = std::max(max_abs_ay_ref, std::fabs(ay_ref));
        sum_sq_ay_ref += ay_ref * ay_ref;
        ++count;
    }
    const double rms_ay_ref = std::sqrt(sum_sq_ay_ref / std::max(1, count));
    const double preview_ay = std::max(max_abs_ay_ref, rms_ay_ref);

    const double risk_raw = (preview_ay - p_.qband_preview_ay_threshold) / p_.qband_preview_ay_range;
    const double r = clamp(risk_raw, 0.0, 1.0);
    const double smooth_r = r * r * (3.0 - 2.0 * r);
    const double scale = lerp(p_.qband_preview_scale_min, p_.qband_preview_scale_max, smooth_r);
    return p_.qband_weight_base * scale;
}
...
bool NMPCAntiExitationController::solveNMPC(const std::vector<double>& current_state,
    ...
    try {
        ...
        opti_.set_value(qband_weight_param_, computePreviewQbandWeight(vx_now, waypoints));
