...
bool ESOTracker2::initialize(ros::NodeHandle& nh) {
    ...
    // 加载—— 参数估计中参数 ——
    nh_nmpc.param("rls_Cf_est", rls_Cf_est_default_, 250.0);          // 挂车前轴刚度初始值（N/m）
    nh_nmpc.param("rls_Cr_est", rls_Cr_est_default_, 1000.0);         // 挂车后轴刚度初始值（N/m）
    nh_nmpc.param("rls_Ct_est", rls_Ct_est_default_, 400.0);          // 挂车横摆转动惯量初始值（kg·m²）
    ...
    rls_C_out_prev_ = Vector3d(rls_Cf_est_default_, rls_Cr_est_default_, rls_Ct_est_default_);
    //  —— IPOPT求解器参数 ——
...
void ESOTracker2::rlsIdentifyStiffness(double curr_vx, double vy_est, double curr_delta,double curr_r, 
    ...
    // 输出侧偏刚度估计值（橙色）
    ROS_INFO("[%s] \033[33m报告！侧偏刚度估计: Cf=%.1f N/rad, Cr=%.1f N/rad, Ct=%.1f N/rad\033[0m", 
