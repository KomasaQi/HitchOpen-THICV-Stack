...
def ACC_PV_dv_callback(msg):
    global PV_dv
    PV_dv = msg.data 
    
def ACC_PV_d_callback(msg):
    global PV_d
    PV_d = msg.data 
#全局变量初始化
PV_d = 0.0
...
def Steering_and_Speed_Ctrl(x_pos,y_pos):
    ...
    #acc_cmd=0.13 #油门开度
        acc_cmd += pid_controller_vel.update(np.real(ref_distance**2.1),np.real(distance_true**2.1))
...
if __name__ =="__main__":
    ...
    PV_d_sub=rospy.Subscriber("/ACC_PV_d",TFMessage,ACC_PV_d_callback,queue_size=1)
    PV_dv_sub=rospy.Subscriber("/ACC_PV_dv",TFMessage,ACC_PV_dv_callback,queue_size=1)
