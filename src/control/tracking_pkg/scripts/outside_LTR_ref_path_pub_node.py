...
# 本节点用于液罐车防侧翻小车实验过程中的路径发布，和原始路径发布节点的区别在于，新的节点会根据当前车速动态调整发布的轨迹的点的数量
import rospy
...
if traj_name == 'Test_sharp':
    ...
    point_interval = 0.1 # 轨迹离散程度 m
    
    x, y = Test_Circle_Gen(dlc_len,dlc_start,dlc_end,dlc_with,left_corner_rad,side_width,rot_angle,line_len,sharpness,point_interval=point_interval)
    refpath_x = x+x_dev
    refpath_y = y+y_dev
