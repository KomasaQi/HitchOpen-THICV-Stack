...
def generate_single_spline_trajectory(point_interval, traj_length, lateral_distance, rotation_angle=0):
    shape = 15.5  # 整体转向剧烈程度
    dx = 5.0    # 平缓程度，越大越平缓
    dy = lateral_distance    # 控制换道y向位置
    Xs = 1.0  # 控制换道距离
    x = np.linspace(0, traj_length, round(traj_length/point_interval)+1)  # 点的数量根据纵向速度x_dot决定

    # 生成参考轨迹
    z = shape/dx * (x - Xs) - shape/2
    y = dy/2. * (1 + np.tanh(z))
...
# 生成单移线轨迹
x_traj, y_traj = generate_single_spline_trajectory(point_interval=0.1, traj_length=10, lateral_distance=2, rotation_angle=np.pi/4)

# 绘制轨迹图像
plt.figure()
plt.plot(x_traj, y_traj, label='Single Spline Trajectory')
plt.xlabel('X')
plt.ylabel('Y')
plt.title('Single Spline Trajectory')
...
if traj_name == 'DLC':
    ...
    traj_length = 20      # 双移线的长度m
    lateral_distance = -0.5  # 横向移动距离m
    rotation_angle = math.atan(direction_tan)  # 旋转角度deg

    x, y = generate_single_spline_trajectory(point_interval, traj_length, lateral_distance, rotation_angle=rotation_angle)
    refpath_x = x
    refpath_y = y
