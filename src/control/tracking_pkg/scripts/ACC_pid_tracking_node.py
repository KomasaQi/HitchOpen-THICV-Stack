...
acc_limit = 0.35
acc_limit_global=0.35

#********根据ACC_ref_speed_planner节点发布的参考速度更新参考速度**********
def acc_refspeed_callback(msg):
    global ref_vel,max_ref_vel
    ref_vel0 = msg.data
    # 对参考速度进行限幅
    if ref_vel0 > max_ref_vel:
        ref_vel0 = max_ref_vel
    elif ref_vel0 <-0.05:
        ref_vel0 = -0.05
    ref_vel = ref_vel0
#*********************************************************************
...
def Steering_and_Speed_Ctrl(x_pos,y_pos):
    ...
    # 笑景修改 2023.12.21 加入AEB功能******
    if (ref_vel < 0)&(vel_true > 0.1):
        acc_cmd = -0.5
        rospy.logerr("TTC小于阈值!紧急制动中!")
    #*************************************
...
def rate_callback(msg):
    ...
    vel_true = (msg.data/360)*WHEEL_RADIUS*0.04 #这里从0.04改成了0.02 小晶注释 20231211
    acc_limit = np.min([acc_limit_global,vel_true*0.46+0.17])




if __name__ =="__main__":
    rospy.init_node("ACC_pid_tracking_node")
    ...
    acc_ref_speed_sub=rospy.Subscriber("/ref_speed",Float32,acc_refspeed_callback,queue_size=1)
    ...
    pid_controller = PID(kp=15.5, ki=0.1, kd=20.5,
