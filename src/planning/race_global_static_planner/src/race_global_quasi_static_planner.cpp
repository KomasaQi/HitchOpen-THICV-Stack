...
race_msgs::Path RaceGlobalQuasiStaticPlanner::generateLocalPath(const race_msgs::VehicleStatus& status)
{
    ...
    // 检查车辆状态是否已接收
    if (!is_vehicle_status_received_)
    {
        ROS_WARN_THROTTLE(1.0, "Cannot generate local path: vehicle status not received yet");
        return local_path;
    }
    ...
    double car_heading = status.euler.yaw;
    ...
    for (size_t i = 0; i < global_path_points_.size(); ++i)
    {
        ...
        double pos_cost = weight_x_ * dx * dx
                        + weight_y_ * dy * dy
                        + weight_z_ * dz * dz;

        double path_heading = tf::getYaw(path_point.pose.orientation);
        double heading_dist = AngleUtils::angle_norm(path_heading, car_heading);
    ...
    double merge_distance = 40.0;      // 越大越平缓，建议先用 6.0
