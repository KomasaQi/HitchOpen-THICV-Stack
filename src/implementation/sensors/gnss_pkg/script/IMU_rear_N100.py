...
# 接收数据线程
def receive_data():
    global Body_acceleration_X, Body_acceleration_Y, Body_acceleration_Z
    global Angular_velocity_X, Angular_velocity_Y, Angular_velocity_Z
    global Roll, Pitch, Heading
    open_port()
    # pub_odom=rospy.Publisher('/trailer/odom',Odometry,queue_size=1)
    pub_imu=rospy.Publisher('/trailer/imu/data',Imu,queue_size=1)
    pub_roll=rospy.Publisher('/trailer/roll',Float32,queue_size=1)
    pub_pitch=rospy.Publisher('/trailer/pitch',Float32,queue_size=1)
    pub_heading=rospy.Publisher('/trailer/heading',Float32,queue_size=1)
    ...
    while serial_.isOpen() and not rospy.is_shutdown() :
        ...
        if head_type == TYPE_IMU:
            ...
            Body_acceleration_X=struct.unpack('f', data_s[12:16])[0]
            Body_acceleration_Y=struct.unpack('f', data_s[16:20])[0]
            Body_acceleration_Z=struct.unpack('f', data_s[20:24])[0]

            Angular_velocity_X=struct.unpack('f', data_s[0:4])[0]
            Angular_velocity_Y=struct.unpack('f', data_s[4:8])[0]
            Angular_velocity_Z=struct.unpack('f', data_s[8:12])[0]
        # 读取并解析AHRS数据
            ...
            Roll=struct.unpack('f', data_s[12:16])[0]
            Pitch=struct.unpack('f', data_s[16:20])[0]
            Heading=struct.unpack('f', data_s[20:24])[0]
        # 读取并解析INSGPS数据
        ...
        imu_msg.header.frame_id='/trailer/imu_link'
        ...
        pub_heading.publish(new_heading*180/PI) # 转换方向，东为0-PI~PI，北为 PI/2
