...
class IMUDataHandler:
    def __init__(self):
        ...
        self.imu_pub = rospy.Publisher('/trailer/imu/data', Imu, queue_size=5)
    ...
    def publish_imu_data(self):
        ...
        rospy.loginfo_throttle(
            1.0,
            "Trailer IMU - 陀螺仪: (%.3f, %.3f, %.3f) rad/s | 加速度计: (%.3f, %.3f, %.3f) m/s²",
        ...
        # 可选：姿态长时间不更新时给出提醒
        now = rospy.Time.now()
        euler_age = (now - self.last_euler_time).to_sec()
        if euler_age > 0.2:
            rospy.logwarn_throttle(
                1.0,
                "Trailer AHRS 姿态超过 %.3f s 未更新，请检查 UKF 欧拉角 CAN 输出(ID=0x%03X)。",
                euler_age,
                UKF_EULER_ID
            )
...
def main():
    ...
    rospy.loginfo("挂车IMU数据发布器启动")

    try:
        bus = can.interface.Bus(channel='can0', bustype='socketcan')
    except Exception as e:
        rospy.logerr("CAN总线初始化失败: %s", str(e))
        return
